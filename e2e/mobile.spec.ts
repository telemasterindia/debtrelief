...
test("mobile menu opens, lists all pages, and closes with Escape", async ({ page, isMobile }) => {
  ...
  for (const label of ["Debt Validation", "How It Works", "Resources", "FAQ", "About", "Contact"]) {
...
test("the review CTA is visible above the fold", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByRole("link", { name: /Request a Validation Review/ }).first()).toBeInViewport();
});
...
test("reduced motion: content is visible without animation", async ({ browser }) => {
  ...
  await page.goto("/");
  await expect(page.getByRole("heading", { name: "Trust Is Earned With Transparency." })).toBeVisible();
  await context.close();
});
